...
HELP = """ Example:
---------------------------------------------------------------------------
R2 sequence with four structures:
cell_bc1              cell_bc2             cell_bc3    linker   sample_index
ACGATTGNNNNNNNNNNNNNNNAAACCGGNNNNNNNNNNNNNNNACCCTAANNNNNNNNNNNNNNNAGANNNNNN
ACGATTGNNNNNNNNNNNNNNNNAAACCGGNNNNNNNNNNNNNNNACCCTAANNNNNNNNNNNNNNNAGANNNNN 
ACGATTGNNNNNNNNNNNNNNNNNAAACCGGNNNNNNNNNNNNNNNACCCTAANNNNNNNNNNNNNNNAGANNNN 
ACGATTGNNNNNNNNNNNNNNNNNNAAACCGGNNNNNNNNNNNNNNNACCCTAANNNNNNNNNNNNNNNAGANNN 
---------------------------------------------------------------------------
Trimming:        |        |
...
class Fastq_transform:
    ...
    max_frags_cutoff_plot = 100000, bins=100, width = 10, height=12, adaptor = ''):
    ...
    def process_R1_R2(self):
        ...
        for R1_field,R2_field in zip(self.__readfq(f_R1),self.__readfq(f_R2)):
            R1_name,R1_seq,R1_qual = self._trimming(R1_field)
            sample_index,temp_cell_bc1,temp_cell_bc2,temp_cell_bc3 = self.__R2_Demultiplexing(R2_field)      
            Total_reads += 1
            if sample_index and len(R1_seq) >= self.min_length_seq:
                ...
                #### To reduce the time-consuming, we write 10000 reads to file one time
                if ii % self.cache_records == 0:  
                    ...
                    self.__write_to_file(buffer_R1, buffer_R2)
                    ii = 1
                    buffer_R1 = []
                    buffer_R2 = []
                else:                
    ...
    def hist_plot(self,all_line = []):
        ...
        i = 0
        j = 0
        ###  for the index of the subplot within compound graph
        comb = []
    ...
    def __write_to_file(self,buffer_R1, buffer_R2):
        ...
        for R1_record,R2_record in zip(buffer_R1,buffer_R2):
            ...
            if outname and ten_x_barcode: 
                if outname not in temp_for_write_R1.keys():
                    ...
                    temp_for_write_R2[outname]  = temp_for_write_R1[outname] + '@' + R1_name + ten_x_barcode + '\n' + '+\n' + 'FFFFFFFFFFFFFFFF\n'
    ...
    def __gen_adaptor(self,adaptor,rev=False):
        ...
        for i in range(len(adaptor)):
            if not rev:
                if len(adaptor[i:len(adaptor)]) > 6:
                    adaptor_subset.append(adaptor[i:len(adaptor)])
            else:
                if len(adaptor[0:i+1]) > 6:
